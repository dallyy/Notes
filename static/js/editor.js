...
export const selectNote = (id, openPreview = true) => {
...
export const saveCurrentNote = async () => {
    ...
    if (title === state.lastSavedTitle && content === state.lastSavedContent)
        return;
    state.saving = true;
...
export const deleteNote = async () => {
    if (!state.currentNoteId)
        return;
    if (!await showConfirm("确定要删除这条笔记吗？此操作无法撤销。"))
        return;
    const result = await apiSafe(`/api/notes/${state.currentNoteId}`, { method: "DELETE" }, "删除失败");
    if (!result)
        return;
