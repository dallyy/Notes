...
let graphRaf = null;
let graphData = null; // { nodes:[...], links:[{s,t}] }
...
const isGraphNeighbor = (idA, idB) => {
    if (idA === idB)
        return true;
    return graphData.links.some((l) => (l.s.id === idA && l.t.id === idB) || (l.s.id === idB && l.t.id === idA));
};
const pickGraphNode = (px, py) => {
    let best = null, bestD = Infinity;
    for (const p of projectAll()) {
        const r = graphNodeRadius(p.n) * Math.max(p.s, 0.3);
        const d = Math.hypot(p.x - px, p.y - py);
        if (d < Math.max(16, r + 5) && d < bestD) {
            bestD = d;
            best = p.n;
        }
    }
...
const drawGraph = () => {
    ...
    if (graphPointer && !(graphDrag && graphDrag.on)) {
        ...
        for (const p of proj) {
            ...
            if (dd < Math.max(16, rr + 5) && dd < bestD) {
                bestD = dd;
                hover = p.n;
            }
        }
    ...
    graphData.links.forEach((l) => {
        ...
        if (hoverId && !isGraphNeighbor(hoverId, l.s.id))
            alpha *= 0.08;
...
export const openGraph = () => {
    ...
    const showIsolated = document.getElementById("graphShowIsolated").checked;
