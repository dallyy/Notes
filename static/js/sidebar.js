...
const persistLocalFolderState = () => {
    storage.set(FOLDERS_KEY, JSON.stringify({ folders: state.folders, noteFolder: state.noteFolder }));
};
...
export const loadFolderState = () => {
    ...
    fetch("/api/folders", { cache: "no-cache" })
        ...
        if (!serverData)
            return;
        const serverHasData = (Array.isArray(serverData.folders) && serverData.folders.length > 0) ||
        ...
        if (serverHasData) {
            ...
            localFolders.forEach((f) => { if (!ids.has(f.id)) {
...
const folderNoteCount = (id) => Object.values(state.noteFolder).filter((fid) => fid === id).length;
...
const makeNoteItem = (n, inFolder) => {
    ...
    if (inFolder)
        li.classList.add("note-in-folder");
    if (n.id === state.currentNoteId)
...
export const renderNoteList = () => {
    ...
    const sortByUpdated = (a, b) => Date.parse(b.updated_at) - Date.parse(a.updated_at);
    ...
    const sortedFolders = [...state.folders].sort((a, b) => String(a.name || "").localeCompare(String(b.name || ""), "zh-CN"));
    sortedFolders.forEach((f) => {
        const fNotes = (byFolder[f.id] || []).slice().sort(sortByUpdated);
        visibleCount += fNotes.length;
        if (q && fNotes.length === 0)
            return; // 搜索时隐藏空文件夹
    ...
    if (visibleCount === 0) {
        noteList.appendChild(el("div", { class: "note-list-empty" }, q ? "无匹配笔记" : "暂无笔记，点击上方按钮创建"));
    }
...
const moveNoteToFolder = (noteId, folderId) => {
    if (state.noteFolder[noteId] === folderId)
        return;
...
const deleteFolder = async (id) => {
    ...
    if (!f)
        return;
    const ok = await showConfirm(`确定要删除文件夹「${f.name || ""}」吗？\n其中的笔记会移到顶层，不会被删除。`);
    if (!ok)
        return;
    for (const [noteId, fid] of Object.entries(state.noteFolder)) {
        if (fid === id)
            delete state.noteFolder[noteId];
    }
